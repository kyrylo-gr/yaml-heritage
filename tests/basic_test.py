...
class BasicTest(unittest.TestCase):
    ...
    def test_sample(self):
        self.chip = elements.Sample.load(
            folder="tests/yaml_files", params="sample.yaml"
        )
        logs = self.draw()
        self.assert_logs(
            logs, ['sample param_int_123 123', 'sample param_str_abc abc']
        )

    def test_parent(self):
        self.chip = elements.Parent.load(
            folder="tests/yaml_files", params="parent.yaml"
        )
        logs = self.draw()
        self.assert_logs(
            logs,
